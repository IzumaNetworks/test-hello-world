...
echo ""
echo "=== Test 1: Run with default greeting ==="
...
docker run --rm --name "$CONTAINER_NAME" -d -p 8080:8080 -e HELLO_NAME="EnvTest" "$IMAGE_NAME"
sleep 2
...
docker run --rm --name "$CONTAINER_NAME" -d -p 8080:8080 "$IMAGE_NAME" --name "CLITest"
sleep 2
